...
const createCategoryController = async (req, res) => {
  try {
    ...
    const { name, description, tax_applicability, tax, tax_type } = req.body;
    ...
    const ifExists = await categories.findOne({ name });
    if (ifExists) {
      return res.status(400).send({
        success: false,
...
const getAllSubcategoriesOfCategoryById = async (req, res) => {
  try {
    ...
    const subcategoriesOfCategoryById = await categories
      .findById(categoryId)
